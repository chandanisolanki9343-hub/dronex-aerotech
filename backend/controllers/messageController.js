...
export const createMessage = async (req, res) => {
  try {
    ...
    // Send confirmation email & admin notification in parallel (await to keep event loop alive)
    try {
      await Promise.all([
      ...
      console.error("Failed to send contact emails:", err.message);
    }
...
export const replyToMessage = async (req, res) => {
  try {
    ...
    // Send email using transporter (await to keep event loop alive)
    try {
      ...
      console.error("Nodemailer failed to send reply email:", mailError.message);
    }
